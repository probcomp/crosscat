...
cdef extern from "RoundedContinuousComponentModel.h":
     cdef cppclass RoundedContinuousComponentModel:
        double score
     ...
     RoundedContinuousComponentModel *new_RoundedContinuousComponentModel "new RoundedContinuousComponentModel" (cpp_map[cpp_string, double] &in_hypers)
     RoundedContinuousComponentModel *new_RoundedContinuousComponentModel "new RoundedContinuousComponentModel" (cpp_map[cpp_string, double] &in_hypers, int COUNT, double SUM_X, double SUM_X_SQ)
     void del_RoundedContinuousComponentModel "delete" (RoundedContinuousComponentModel *ccm)

cdef class p_RoundedContinuousComponentModel:
    cdef RoundedContinuousComponentModel *thisptr
    ...
    def __cinit__(self, in_map, count=None, sum_x=None, sum_x_squared=None):
          ...
          if count is None:
              self.thisptr = new_RoundedContinuousComponentModel(self.hypers)
          else:
              self.thisptr = new_RoundedContinuousComponentModel(self.hypers,
                                                          count, sum_x, sum_x_squared)
    def __dealloc__(self):
        del_RoundedContinuousComponentModel(self.thisptr)
