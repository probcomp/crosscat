...
## Purpose of revision
Encapsulation, readability and, as a result, extensibility. Adding functionality should require editing in as few places as possible. This reduces the difficulty of editing the code and writing tests. In addition, the engine revision is an opportunity to write comprehensive unit tests and to implement concurrency within the C++ engine (using [OpenMP](http://en.wikipedia.org/wiki/OpenMP)).
...
### State
This python engine will interact only with the state object. This removes the overhead associated with constructing component models from the metadata. Both `simple_predictive_probability` and `simple_predictive_sample` will be moved to `state`.
...
### View
Represents a crosscat view. Views contain vectors of `feature` objects. The view object will represent the concept of the cluster, or category, implicitly. The `cluster` object will no longer exist. Its functionality will be shared by the `feature` and the  `view`. The reasoning is that the bulk of the work the view does is the row transition, and the bulk of the work donw by the row transition operation is calculating the probabilities of different rows of data under different clusters (component models). The row probability can be calculated by summing the log probabilities of the data in row r under the component models of cluster k, so the cluster does not need to be represented in the code and `cluster_lookup` is no longer needed. For example:
...
### Numerics
`numerics` will no longer contain methods for each component class (these methods will be moved to their respective classes) but will be used for general, widely-used methods such as drawing an index from a vector, or calculating the log CRP.
...
# Development 
The development will proceed as a incremental refactoring. At each step, the engine will be left in a working state. This method allows us to run incremental inference quality testing and to easily pinpoint bugs.

The development will proceed in the following steps:
